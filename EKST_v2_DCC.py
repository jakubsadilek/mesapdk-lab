    ...
    for width in widths:
        for gap in gaps_nm:
            for ratio in ratios:
                ...
                dcc_labels.append("{}: W:{:.2f} um G:{:.0f} nm R:{:.2f} L:{:.1f} um".format(len(dcc_labels), width, gap, ratio, dc_lenght))


    dcc_arr = d.add_ref(array_with_y_span(components=dcc_list,
                      pitch_x=381, 
                      y_span=3000, 
                      ...
                      labels=dcc_labels,
                      text = label_txt
                      )).drotate(90)#.dmovey(origin=0, destination=1000) 

    #print(dcc_list)
# 2 / route ports

    dcc_ports1 = dcc_arr.ports.filter(regex = r'^\d+_o[34]$')
    dcc_ports2 = dcc_arr.ports.filter(regex = r'^\d+_o[12]$')
    routes = []
